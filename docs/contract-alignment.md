...
| Metadata preservation | Implemented at the public codec boundary | The pinned request matrix round-trips content type, payload, deadline, causation and `plenora.trace.correlation_id` across database, data, IO and REST selectors. Routing fields are removed only from the adapter-owned input view and restored on encode. |
